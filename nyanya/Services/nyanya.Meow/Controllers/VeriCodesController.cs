...
using nyanya.Meow.Models.VeriCodes;
using System.Net;
...
namespace nyanya.Meow.Controllers
{
    ...
    public class VeriCodesController : ApiControllerBase
    {
        ...
        public async Task<IHttpActionResult> Send(SendVeriCodeTokenRequest request)
        {
            ...
            //FIXME 兼容以前版本的mobile app
            //if (this.CurrentUser.Identifier.IsNullOrEmpty() || request.Cellphone != this.CurrentUser.Cellphone)
            if ((this.CurrentUser.Identifier.IsNullOrEmpty() || request.Cellphone != this.CurrentUser.Cellphone) && request.Token.IsNotNullOrEmpty())
            {
        ...
        {
            //  "Client-Id"不为空表示请求来自手机app，此处不区分手机种类
            ...
            SendVeriCodePhoneResult result = await this.veriCodeService.SendGraphicAsync(identifier, width, clientId);
        ...
        public async Task<IHttpActionResult> VerifyGraphicCode(VerifyVeriCodePhoneRequest request)
        {
            ...
            VerifyVeriCodeResult result = await this.veriCodeService.VerifyGraphicAsync(request.Cellphone, identifier, request.Result, clientId);
