// FileInformation: nyanya/nyanya.Xingye/OrderInfoController.cs
...
namespace nyanya.Xingye.Controllers
{
    ...
    /// </summary>
    [RoutePrefix("Orders")]
    public class OrderInfoController : XingyeApiControllerBase
    {
        private readonly IBAOrderInfoService baOrderInfoService;
        private readonly IOrderInfoService orderInfoService;
        private readonly IXYOrderInfoService xyOrderInfoService;
        ...
        public OrderInfoController(IBAOrderInfoService baOrderInfoService, IXYOrderInfoService taOrderInfoService, IOrderInfoService orderInfoService)
        {
            this.baOrderInfoService = baOrderInfoService;
            this.xyOrderInfoService = taOrderInfoService;
        ...
        ///     xingye票订单列表
        /// </summary>
        /// <param name="endDate">endDate(yyyy-MM-dd): 结束日</param>
        /// <param name="pageIndex">pageIndex(int >= 1): 页码</param>
        /// <param name="startDate">startDate(yyyy-MM-dd): 起始日</param>
        /// <param name="orderMode">orderMode(int): 所需订单状态，1=》all, 2=> 成功, 3=> 失败</param>
        /// <returns>
        ///     HasNextPage: 是否有下一页
        ///     PageIndex: 页码
        ///     PageSize: 一页节点数量
        ...
        ///     -- Yield[decimal]: 订单的预期收益率
        /// </returns>
        [HttpGet, Route("XingYe/Page")]
        [RangeFilter("pageIndex", 1)]
        [RangeFilter("orderMode", 1, 3)]
        [TokenAuthorize]
        [ResponseType(typeof(OrderListResponse))]
        public async Task<IHttpActionResult> GetXingYeOrders(string startDate = "", string endDate = "", int pageIndex = 1, int orderMode = 1)
        {
            DateTime startTime = new DateTime();
            DateTime endTime = DateTime.MaxValue;
            OrderMode mode = OrderMode.AllOrder;
            if (startDate.IsNotNullOrEmpty() && !DateTime.TryParse(startDate, out startTime))
            {
                return this.BadRequest("error startDate format");
            }
            if (endDate.IsNotNullOrEmpty() && !DateTime.TryParse(endDate, out endTime))
            {
                return this.BadRequest("error endDate format");
            }
            if (orderMode == 2)
            {
                mode = OrderMode.SuccessfulOrder;
            }
            else if (orderMode == 3)
            {
                mode = OrderMode.FailedOrder;
            }
            IPaginatedDto<OrderInfo> orders =
                await
                    this.xyOrderInfoService.GetOrderInfosAsync(this.CurrentUser.Identifier, startTime, endTime, pageIndex,
                        mode);
            return this.Ok(new OrderListResponse(orders));
        }
    }
