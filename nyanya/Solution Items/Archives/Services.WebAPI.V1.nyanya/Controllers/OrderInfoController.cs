// FileInformation: nyanya/Services.WebAPI.V1.nyanya/OrderInfoController.cs
// CreatedTime: 2014/08/10   1:23 PM
// LastUpdatedTime: 2014/08/15   2:52 PM
...
namespace Services.WebAPI.V1.nyanya.Controllers
{
    ...
    public class OrderInfoController : ApiControllerBase
    {
        ...
        public OrderInfoController(IBAOrderInfoService baOrderInfoService, ITAOrderInfoService taOrderInfoService, IOrderInfoService orderInfoService)
        {
        ...
        ///     -- Yield[decimal]: 订单的预期收益率
        /// </returns>
        [HttpGet, Route("TA/Failed")]
        [TokenAuthorize]
        [ResponseType(typeof(OrderListResponse))]
        public async Task<IHttpActionResult> GetTAFailedOrders()
        {
            IPaginatedDto<TAOrderInfo> orders = await this.taOrderInfoService.GetFailedOrderInfosAsync(this.CurrentUser.Identifier);
        ...
        ///     -- Yield[decimal]: 订单的预期收益率
        /// </returns>
        [HttpGet, Route("TA"), Route("TA/{pageIndex:min(1):int=1}/{sortMode:min(1):int=1}")]
        [RangeFilter("pageIndex", 1)]
        [TokenAuthorize]
        [ResponseType(typeof(OrderListResponse))]
        public async Task<IHttpActionResult> GetTAOrders(int pageIndex = 1, int sortMode = 1)
        {
            OrderSortingStrategy sortingStrategy = sortMode == 1 ? OrderSortingStrategy.ByOrderTime : OrderSortingStrategy.BySettleDate;

            IPaginatedDto<TAOrderInfo> orders = await this.taOrderInfoService.GetSuccessfulOrderInfosAsync(this.CurrentUser.Identifier, pageIndex, 10, sortingStrategy);
