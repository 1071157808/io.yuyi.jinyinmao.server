define(['jym', 'dialog'], function(jym, dialog){
    ...
    detail.progress = {
        ...
        bind:function(){
            var o = $('.bank-reduce-progress .percent em');
            var t = $('.bank-reduce-progress .text b');
    ...
    detail.lastCount = {
        ...
        render:function(){
            $('.bank-reduce-part b').text(_available);
            $('#paymentCount').trigger('blur');
            var cent = parseInt((_paid/_sum) * 100);
            var progress = $('.bank-reduce-progress');
            var bar = progress.find('em');
            var num = progress.find('i');
            bar.data('value', cent);
    ...
    detail.tabs = {
        ...
        change:function(tab){
            $('.bank-tab-list li.on').removeClass('on');
            $('.bank-tab-items:visible').hide();
            var tab_id = tab.getAttribute('data-tab');
            if(!document.getElementById(tab_id)){
                var txt = $('#tpl-'+tab_id).val();
                $('.bank-tab-content').append(txt);
            } else {
                $('#'+tab_id).show();
            }
