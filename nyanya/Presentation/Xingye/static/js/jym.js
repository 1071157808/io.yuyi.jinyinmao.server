(function(root, factory) {
    ...
    var env_name = "Dev";
    if(/piao\.cib\.com/.test(hostname)){
        env_name = "Prd";
    } else if(/xy\.test\.jymtest\.com/.test(hostname)){
        env_name = "Test";
    }
    ...
    $.extend(jym, {
        ...
        formPost:function(url, data, target){
            var tpl = '<form action="'+url+'" method="post" '+(target?'target="_blank"':'')+' style="display:none;">';
        ...
        msgError:function(selector, msg, visable){
            var jq = $(selector), tip = jq.siblings('.ui-tip-span');
            if(visable != false){
                if(!tip.length){
                    var txt = '<span class="ui-tip-span"><span class="ui-tip-msg">';
                    txt += '<span class="ui-ico-error"></span><b class="ui-tip-left"></b></span></span>';
                    tip = $(txt);
                    jq.after(tip);
                }
                tip.show().find('span.ui-ico-error').html(msg);
            } else if(tip.length) {
                tip.hide();
            }
        },
        msgOk:function(selector, visable){
            var jq = $(selector), tip = jq.siblings('.ui-ico-ok');
            if(visable != false){
                if(!tip.length){ tip = $('<span class="ui-ico-ok"></span>'); jq.after(tip); }
                tip.show()
            } else if(tip.length){
                tip.hide();
            }
        },
        msgInfo:function(msg, visable){
            var jq = $('#ui-msg-info');
            if(visable != false){
                if(jq.length){ jq.html('<p class="ui-ico-error">'+msg+'</p>').show(); }
            } else {
                jq.hide();
            }
        },
    ...
    if ("placeholder" in document.createElement("input")) {
        ...
        jym.placeholder.trigger = function(selector){
            $(selector).next('[data-type=hidden]').trigger('focus');
        };
        $(function(){ jym.placeholder('input[placeholder]') });
    }
    ...
    jym.notice = {
        init:function(){
            var that = this,
                el = document.getElementById('top-marquee');
//            if(el){this.render(el);}
            if(!el) { return }
            jym.notice.setMarquee(el);
            $('#noticeHide').click(function(){
                that.close($(this));
            });
        },
        render:function(mq){
            /*
            require(['/Notice/Public'], function(d){
                if(d){
        ...
        close : function(obj){
            obj.closest('.ui-notice-wrap').fadeOut();
        },
        txweix : function(){
            $("#company .tx-wei").hover(
                function(){$(".company .ercode").show()},
                function(){$(".company .ercode").hide()}
            )
        },
        setMarquee:function(obj){
            var width = obj.offsetWidth, i = 0, delay = 30, parent_w = obj.parentNode.offsetWidth;
    ...
    jym.api.passport = {
        ...
        hasLoginPassword:"/api/v1/User/HasLoginPassword?cellphone={cellphone}",
        setLoginPassword:"/api/v1/User/SetLoginPassword",
        validPayment:'/api/v1/User/VerifyPaymentInfo',
        ResetPaymentPassword:"/api/v1/User/ResetPaymentPassword",
        AnyValidPayment:'/api/v1/User/AnyVerifyPaymentInfo',
        AnyResetPaymentPassword:'/api/v1/User/AnyResetPaymentPassword',
        tempSignUp:"/api/v1/User/TempSignUp"
    };
    jym.api.sms = {
    ...
    jym.load = {
        login_tpl:function(){
            var tpl = '<a class="ui-nav-login" href="/user/index">我的兴业票</a>';
            tpl += '<a class="ui-nav-register" href="#" onclick="jym.load.logout();return false;">退出</a>';
            return tpl;
        },
