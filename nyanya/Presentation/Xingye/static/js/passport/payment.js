define(['jym'], function(jym){
    var resetAjax = function(data, callback){
        jym.sync({
            type:"post",
            url:jym.api.passport.AnyResetPaymentPassword,
            data:data,
            success:callback,
            error:function(d){
    ...
    var obj = {
        ...
        bind:function(){
            jym.action.extend({
                ...
                "prev-page":function(){ history.back(-1) }
            });
            ...
            $('#getCode').click(function(){
                ...
                obj.validate.mobile(function(mobile){
                    jym.sendValidateCode(mobile, '30', function(d, num){
                        if(d == -1){
        ...
        checkPaymentCard:function(callback){
            obj.validate.mobile(function(mobile){
                ...
                if(status.bankCard && status.idCard){
                    jym.sync({
                        type:'post',
                        url:jym.api.passport.AnyValidPayment,
                        data:{
                        ...
                        success:function(){
                            jym.fire(callback, [status.bankCard, status.idCard, mobile]);
                        },
                        error:function(d){
                            jym.msgInfo(d.responseJSON.Message);
                        }
        ...
        resetPayment1:function(){
            obj.checkPaymentCard(function(bankCard, idCard, mobile){
                obj.validate.code(function(token){
                    $('#validBtn').removeClass('ui-btn1').addClass('ui-btn');
                    var url = "/passport/reset-payment-2?token={token}&bankcard={bankcard}&idcard={idcard}&mobile={mobile}";
                    var d = {token:token, bankcard:bankCard, idcard:idCard, mobile:mobile};
        ...
        resetPayment2:function(){
            if(obj.validate.password1() && obj.validate.password2()){
                ...
                resetAjax(d, function(){
                    location.href = '/passport/reset-payment-3';
                });
            }
        }
    };
    obj.validate = {
        ...
        password1:function(){
            ...
            if(!jym.isPayPassword(pwd)){
                jym.msgError($pwd, '8-18位字母、数字和符号(~!@@#$%^&*_)的组合，区分大小写。');
                return false;
            }
