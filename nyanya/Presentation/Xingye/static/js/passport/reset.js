define(['jym'], function (jym) {
    ...
    var obj = {
        ...
        getMobileSource:function(callback){
            if(typeof _mobile != 'undefined' && !document.getElementById("username")){
                jym.fire(callback, [_mobile]);
            }
        },
        bind:function(){
            var that = this, mobile = status.mobile;
            jym.action.extend({
                "reset-step1":function(){ obj.resetStep1(); },
                "reset-step2":function(){ obj.resetStep2(); },
                "prev-password":function(){ obj.prev_password(); },
                "prev-page":function(){ history.go(-1); }
            });
            ...
            $('#getCode').click(function(){
                ...
                obj.getMobileSource(function(_mobile){
                    mobile = _mobile;
                });
        ...
        resetStep2:function(){
            ...
            if(!pwd2){ return; }
            if(status.password && status.token){
                resetAjax({
                    CellPhone:status.mobile,
                    Password:status.password,
                    Token:status.token
                }, function(){
                    location.href = '/passport/reset-password-2';
                });
            }
        },
        ...
        showTitle:function(){
            if(typeof _mobile != 'undefined'){
                if(typeof _hasBankCard != 'undefined' && !_hasBankCard){
                    $('#resetTitle').html('<h1>重置登录密码</h1>');
                    return false;
                }
                jym.hasPassword(_mobile, function(d){
                    var keys = '';
                    if(d.Result){
                        keys = '重置';
                    } else {
                    ...
                    $('#resetTitle').html('<h1>' + keys + '登录密码</h1>');
                });
            } else {
                if(jym.urlQuery('type')=='payment'){
                    $('#resetTitle').html('<h1>设置登录密码</h1>');
                } else {
                    $('#resetTitle').html('<h1>重置登录密码</h1>');
                }
            }
    ...
    obj.validate = {
        ...
        code:function(codeCallback){
            ...
            chkMobile(function(mobile){
                if(that.ValidCode() == false){ return false; }
