...
// CreatedTime: 2014/08/26   2:06 PM
// LastUpdatedTime: 2014/08/27   12:43 PM
...
public static class CatHelper
{
    ...
    public const bool IsCatFlag = true;
    ...
    public static string FixIdCard(object id, int first = 5, int last = 4)
    {
        string str = id.ToString();
        string start = str.GetFirst(first);
        string end = str.GetLast(last);
        return string.Format("{0}********{1}", start, end);
    }
    ...
    public static NyanyaWebData GetData(this HttpRequestBase request, string route)
    {
        ...
        try
        {
            ...
            RestRequest restRequest = new RestRequest(ApiPath.Remove(0, 1) + route.Remove(0, 1), Method.GET);
            restRequest.RequestFormat = DataFormat.Json;
            restRequest.AddHeader("Content-Type", "application/json");
            client.CookieContainer = cookieContainer;
            var response = client.Execute<dynamic>(restRequest);
    ...
    public static NyanyaWebData[] GetDatas(this HttpRequestBase request, params string[] routes)
    {
        ...
        RestRequest BatchRequest = new RestRequest(ApiPath.Remove(0, 1) + BatchRoute, Method.POST);
        ...
        {
            Body.Add(new { Method = "GET", RelativeUrl = ApiPath.Remove(0, 1) + i.Remove(0, 1) });
        }
        ...
        if (DebugEnable != null && DebugEnable.ToUpper() == "T" && !string.IsNullOrEmpty(request.QueryString["debug"]))
        {
            foreach (NyanyaWebData data in ListData)
            {
                if (request.QueryString["debug"] == "1")
                {
                    HttpContext.Current.Response.Write(string.Format("<!--{0}|{1}-->", JsonConvert.SerializeObject(data)));
                }
                else if (request.QueryString["debug"] == "2")
                {
                    HttpContext.Current.Response.Write(string.Format("<!--{0}|{1}-->", JsonConvert.SerializeObject(new { data.Data, data.Message, data.Result, data.Status })));
                }
                else
                {
                    HttpContext.Current.Response.Write(string.Format("<!--{0}|{1}-->", JsonConvert.SerializeObject(data.Data)));
                }
            }
        }
        return ListData.ToArray();
    ...
    public static bool UserPasswordCheck(dynamic userInfo){
        if(userInfo.HasLoginPwd == false){
            HttpContext.Current.Response.Redirect("~/passport/reset-password-1");
            return false;
        }
        if(userInfo.HasDefaultBankCard == false){
            HttpContext.Current.Response.Redirect("~/user/bank/add");
            return false;
        }
        return true;
    }
    ...
    {
        return (price * yield * period) / (year * 100);
    }
    ...
    public static CookieCollection ToCookieCollection(this HttpCookieCollection cookies, string domain)
    {
        ...
        {
            System.Web.HttpCookie cookie = cookies.Get(j);
