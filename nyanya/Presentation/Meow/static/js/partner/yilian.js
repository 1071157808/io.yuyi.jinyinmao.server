zim.ns('zim.page.yilian.valid', {
    ...
    hasCard:function(d){
        if(d.HasDefaultBankCard){
            zim.location('/partner/yilian/addCard');
        }
    }
});
...
zim.ns('zim.page.yilian.identity', {
    ...
    nextAddCard:function(){
        ...
        zim.location('/partner/yilian/addCard' + p);
    }
});
zim.ns('zim.page.yilian.addCard', {
    ...
    submit:function(d, api){
        zim.sync({
            ...
            data:d,
            success:function(){
                zim.location('/partner/yilian/verify?no=' + d.BankCardNo);
            },
            error:function(d){
                zim.ui.alert(d.responseJSON.Message);
            }
...
zim.ns('zim.page.yilian.verify', {
    init:function(){
        this.bind();
    },
    bind:function(){
        var that = this, card = zim.urlQuery("no");
        zim.action.extend({
            "return-buy":function(){
                that.getCardStatus(card);
            }
        });
    },
    getCardStatus:function(card){
        ...
        zim.sync({
            type:"post",
            url:zim.api.bankcard.AddBankCardStatus,
            data:{BankCardNo:card},
            success:function(d){
                if(d.Status == 1){
                    that.location_buy();
                } else if(d.Status == 2) {
                    zim.ui.message('绑卡认证中，请稍候查询。');
                } else {
                    zim.ui.alert(d.Message, function(){
                        zim.location("/partner/yilian/identity");
                    });
                }
            },
