// FileInformation: nyanya/Infrastructure.Lib/FixedLengthConcurrentQueue.cs
...
namespace Infrastructure.Lib
{
    ...
    [Serializable]
    public class FixedLengthConcurrentQueue<T> : IProducerConsumerCollection<T>
    {
        ...
        /// </summary>
        public FixedLengthConcurrentQueue()
        {
            this.m_head = this.m_tail = new Segment(0, this);
        }
        ...
        ///     The <paramref name="collection" /> argument is
        ///     null.
        /// </exception>
        [Obsolete]
        public FixedLengthConcurrentQueue(IEnumerable<T> collection)
        {
        ...
        public FixedLengthConcurrentQueue(int size, IEnumerable<T> collection)
        {
            this.MaxSize = size;
            if (null == collection)
            {
            ...
            else
            {
                ...
                {
                    throw new Exception("collection size error");
                }

                InitializeFromCollection(enumerable); 
            }
        ...
        ///     available space from <paramref name="index" /> to the end of the destination
        ///     <paramref
        ///         name="array" />
        ///     .
        /// </exception>
        ...
        ///     The one-dimensional <see cref="T:System.Array">Array</see> that is the
        ///     destination of the elements copied from the
        ///     <see cref="T:System.Collections.Concurrent.ConcurrentBag" />. The
        ///     <see
        ///         cref="T:System.Array">
        ///         Array
        ///     </see>
        ...
        /// <remarks>
        ///     For <see cref="ConcurrentQueue{T}" />, this operation will always add the object to the
            ...
            out int headLow, out int tailHigh)
        {
            ...
            while (
                ...
                head != this.m_head || tail != this.m_tail
        ...
        {
            Contract.Assert(this.m_serializationArray != null);
        ...
        /// <summary>
        ///     Copies the <see cref="ConcurrentQueue{T}" /> elements to a new
        ///     <see
        ///         cref="T:System.Collections.Generic.List{T}" />
        ///     .
        /// </summary>
        /// <returns>
        ///     A new <see cref="T:System.Collections.Generic.List{T}" /> containing a snapshot of
        ///     elements copied from the <see cref="ConcurrentQueue{T}" />.
        /// </returns>
        ...
        private class Segment
        {
            ...
            private volatile FixedLengthConcurrentQueue<T> m_source;
            internal volatile VolatileBool[] m_state;

            /// <summary>
            ///     Create and initialize a segment with the specified index.
            /// </summary>
            internal Segment(long index, FixedLengthConcurrentQueue<T> source)
            {
                this.m_array = new T[SEGMENT_SIZE];
                this.m_state = new VolatileBool[SEGMENT_SIZE]; //all initialized to false
                this.m_high = -1;
            ...
            internal int High
            {
                get
                {
                    ...
                    return Math.Min(this.m_high, SEGMENT_SIZE - 1);
                }
            }
            ...
            internal void Grow()
            {
                ...
                Segment newSegment = new Segment(this.m_index + 1, this.m_source); //m_index is Int64, we don't need to worry about overflow
                this.m_next = newSegment;
                Contract.Assert(this.m_source.m_tail == this);
            ...
            internal bool TryAppend(T value)
            {
                ...
                {
                    if (this.m_source.Count == this.m_source.MaxSize)
                    {
                        while (!this.m_source.m_head.IsEmpty)
                        {
                            if (this.m_source.m_head.TryRemove())
                                break;
                            //since method IsEmpty spins, we don't need to spin in the while loop
                        }
                    }

                    newhigh = Interlocked.Increment(ref this.m_high);

                    if (newhigh <= SEGMENT_SIZE - 1)
                    {
                        this.m_array[newhigh] = value;
                        this.m_state[newhigh].m_value = true;
                    }
                    if (newhigh == SEGMENT_SIZE - 1)
                    {
                        this.Grow();
            ...
            {
                result = default(T);
                int lowLocal = this.Low;
                if (lowLocal > this.High)
                    return false;
                SpinWait spin = new SpinWait();
                while (!this.m_state[lowLocal].m_value)
                {
                    spin.SpinOnce();
                }
            ...
            {
                SpinWait spin = new SpinWait();
                int lowLocal = this.Low, highLocal = this.High;
                while (lowLocal <= highLocal)
                {
                    //try to update m_low
                    if (Interlocked.CompareExchange(ref this.m_low, lowLocal + 1, lowLocal) == lowLocal)
                    {
                        ...
                        {
                            this.m_array[lowLocal] = default(T); //release the reference to the object.
                        }
                        ...
                        if (lowLocal + 1 >= SEGMENT_SIZE)
                        {
                            ...
                            Contract.Assert(this.m_source.m_head == this);
            ...
            {
                Contract.Assert(this.m_high < SEGMENT_SIZE - 1);
                this.m_high++;
