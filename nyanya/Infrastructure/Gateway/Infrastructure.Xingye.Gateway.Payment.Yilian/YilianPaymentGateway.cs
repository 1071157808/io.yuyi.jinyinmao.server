// FileInformation: nyanya/Infrastructure.Xingye.Gateway.Payment.Yilian/YilianPaymentGateway.cs
...
namespace Infrastructure.Xingye.Gateway.Payment.Yilian
{
    ...
    {
        private const string QueryAuthRequestWithParame = "{0}?p={{BATCH_NO:\"{1}\"}}";
        ...
        public async Task<RequestQueryResult> QueryRequestAsync(string batchNo, bool isPayment)
        {
            ...
            try
            {
                ...
                string url = QueryAuthRequestWithParame.FormatWith(QueryAuthRequestUrl, batchNo);
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    {
                        Result = queryResult,
                        Message = string.IsNullOrEmpty(YinlianErrorCodeCache.TryGetErrorMessage(result.ToUpperInvariant())) ? message : YinlianErrorCodeCache.TryGetErrorMessage(result.ToUpperInvariant()),
                        ResponseString = responseString
                    };
                }
            ...
            await SmsAlertsService.AlertAsync(NyanyaResources.Alert_YilianGateway_RequestSendingFailed.FormatWith(json));
            //eturn new RequestQueryResult { Message = message, Result = false, ResponseString = responseString };
            return null;
        }
